...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var state = {},
      deadline = '2021-12-31';
  Object(_modules_changeState__WEBPACK_IMPORTED_MODULE_4__["default"])(state);
...
var forms = function forms(state) {
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            document.querySelector('.status').textContent = message.loading; // let res = await fetch(url,{         просто вариант поста с фетчем, без отправки даты прям в датабазы, было изменено на пост с библиотекой эксиос, для отправки даты в дб.
            //     method: 'POST',
            //     body: data
            // });

            _context.next = 3;
            return regeneratorRuntime.awrap(axios__WEBPACK_IMPORTED_MODULE_7___default.a.post(url, {
              'user_name': data.name,
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.appendChild(statusMessage); // const formData = new FormData(item);
      ...
      postData('http://localhost:3000/requests', formData).then(function (res) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        windows.forEach(function (item) {
          /* закрываем все модальные окна, дальше по коду используем кнопкой close и нажатием на подложку */
          item.style.display = 'none';
        });
...
var tabs = function tabs(headerSelector, tabsSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    var target = e.target; // 1 вариант, с проверкой класса у таргета и его родителя с помощью регулярных выражений(замена точки у класса на ничего, из за метода класлист который знает что это класс)
